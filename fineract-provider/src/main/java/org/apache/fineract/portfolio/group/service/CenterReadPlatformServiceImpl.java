...
public class CenterReadPlatformServiceImpl implements CenterReadPlatformService {
    ...
    private String getCenterExtraCriteriaforAdvanceSearch(String schemaSl, List<Object> paramList,final AdvanceSearchParameters advanceSearchParameters) {
    	StringBuffer extraCriteria = new StringBuffer(200);
        extraCriteria.append(" and g.level_id = " + GroupTypes.CENTER.getId());

        String sqlQueryCriteria = advanceSearchParameters.getSqlSearch();
        
        final String name = advanceSearchParameters.getDisplayName();
        if (name != null) {
        	//paramList.add(ApiParameterHelper.sqlEncodeString("%" + name + "%"));
            //extraCriteria.append(" and g.display_name like ? ");
        ...
        final String externalId = advanceSearchParameters.getCenterExternalId();
        if (externalId != null) {
        	//paramList.add(ApiParameterHelper.sqlEncodeString(externalId));
            extraCriteria.append(" and g.external_id like '%"+externalId+"%'");
        }
        ...
        if (status != null) {
            ...
            extraCriteria.append(" and g.status_enum = ? ");
        }
        
        final Long staffId = advanceSearchParameters.getLoanOfficer();
        if (staffId != null) {
        	paramList.add(staffId);
            extraCriteria.append(" and g.staff_id = ? ");
        }
    ...
    @Override
	public Page<CenterData> advanceRetrievePagedAll(AdvanceSearchParameters advanceSearchParameters,PaginationParameters parameters) {
        ...
        final String extraCriteria = getCenterExtraCriteriaforAdvanceSearch(this.centerMapper.schema(), paramList, advanceSearchParameters);
        ...
        //searchParameters.isOrderByRequested()
        if (parameters.isOrderByRequested()) {
            sqlBuilder.append(" order by ").append(advanceSearchParameters.getOrderBy()).append(' ').append(advanceSearchParameters.getSortOrder());
        }
