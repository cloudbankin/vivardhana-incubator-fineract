...
import org.apache.fineract.portfolio.interestratechart.exception.InterestRatePeriodValidateException;
...
public class InterestRateChartDataValidator {
	...
	public void validateForCreate(final String json, final DataValidatorBuilder baseDataValidator) {
		...
		// interest period validation jai
		...
		Boolean isPrimaryGroupingByAmount = this.fromApiJsonHelper
				.extractBooleanNamed(isPrimaryGroupingByAmountParamName, element);
			...
			if (topLevelJsonElement.has(chartSlabs) && topLevelJsonElement.get(chartSlabs).isJsonArray()) {
				...
				for (int i = 0; i < array.size(); i++) {
					...
					if (this.fromApiJsonHelper.parameterExists(idParamName, interstRateChartElement)) {
						...
						this.chartSlabDataValidator.validateChartSlabsUpdate(interstRateChartElement, baseDataValidator,
								locale, isPrimaryGroupingByAmount);
					} else {
						this.chartSlabDataValidator.validateChartSlabsCreate(interstRateChartElement, baseDataValidator,
								locale, isPrimaryGroupingByAmount);
					}
				}
			}
