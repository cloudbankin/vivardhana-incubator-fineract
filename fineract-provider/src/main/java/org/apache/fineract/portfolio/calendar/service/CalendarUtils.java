...
public class CalendarUtils {
    ...
    public static boolean isValidRecurringDate(final Recur recur, final LocalDate seedDate, final LocalDate date,
        ...
        //Habile added for the collection sheet meeting date and repayment date fix
        if(recurDate == null || recurDate.isEmpty())
        {     	
            int monthsBetween = Months.monthsBetween(seedDate, date).getMonths();
	        
	        LocalDate expectedDate = seedDate.plusMonths(monthsBetween);
	        
	        if(expectedDate.equals(date))
	        {
	        	recurDate.add(expectedDate);
	        }
        }
    ...
    public static LocalDate getNextRepaymentMeetingDate(final String recurringRule, final LocalDate seedDate,
        ...
        //Arivu Anna Changes 28 days problem
        if (frequency.equals(Recur.MONTHLY) && recur.getFrequency().equals(Recur.MONTHLY)) 
        {
	       	 if(repaymentDate.getMonthOfYear()>=2)
	       	 {
	       		newRepaymentDate = newRepaymentDate.withDayOfMonth(seedDate.getDayOfMonth());
	       	 }
       	}
