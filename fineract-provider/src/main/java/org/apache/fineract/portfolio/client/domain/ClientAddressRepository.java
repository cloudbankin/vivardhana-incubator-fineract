...
public interface ClientAddressRepository
	...
	@Query("select ca from ClientAddress ca left join ca.client c where c.id = :clientId")
	ClientAddress getData(@Param("clientId")Long clientId);
	
	@Query("select addr from ClientAddress addr where addr.client.id=:id order by addr.addressType.id desc")
	List<ClientAddress> findAddressByClient(@Param("id") Long id);
}
