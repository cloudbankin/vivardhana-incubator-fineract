...
//			stateIdobj = this.codeValueRepository.getOne(stateId);
			stateId =command.stringValueOfParameterNamed("stateProvinceId");
		}
		...
		final Address add = Address.fromJson(command, stateId, countryIdObj);
		this.addressRepository.save(add);
		final Long addressid = add.getId();
		final String pincode = add.getPostalCode();
		final Address addobj = this.addressRepository.getOne(addressid);
/*		CountryStateDetails csd = this.countryStateRepository.findBypincode(Long.parseLong(pincode));
		addobj.setStateProvince(csd.getState());
		addobj.setCity(csd.getDistrict());*/
...
//				stateIdobj = this.codeValueRepository.getOne(stateId);
				stateId =jsonObject.get("stateProvinceId").getAsString() ;
			}
			...
			final Address add = Address.fromJsonObject(jsonObject, stateId, countryIdObj);
			this.addressRepository.save(add);
			final Long addressid = add.getId();
			final String pincode = add.getPostalCode();
			final Address addobj = this.addressRepository.getOne(addressid);
	...
	public CommandProcessingResult updateClientAddress(final Long clientId, final JsonCommand command) {
		...
		if (!(command.stringValueOfParameterNamed("stateProvinceId").isEmpty())) {
			//	if ((command.stringValueOfParameterNamed("stateProvinceId") != null)) {
//					if ((command.stringValueOfParameterNamed("stateProvinceId") != 0)) {
						is_address_update = true;
//						stateId = command.longValueOfParameterNamed("stateProvinceId");
//						stateIdobj = this.codeValueRepository.getOne(stateId);
						stateId =command.stringValueOfParameterNamed("stateProvinceId");
						addobj.setStateProvince(stateId);
						//addobj.setStateProvince(stateIdobj);
//					}
