...
public class LoanWritePlatformServiceJpaRepositoryImpl implements LoanWritePlatformService {
    ...
    //Habile added for checking the nominee details before disbursment
    ...
    public LoanWritePlatformServiceJpaRepositoryImpl(final PlatformSecurityContext context,
            ...
            final SmsProcessingService smsProcessingService) {
    ...
    public CommandProcessingResult disburseLoan(final Long loanId, final JsonCommand command, Boolean isAccountTransfer) {
    	...
    	//Habile changes for checking wether the client has nominee details before disbursing loan 
    	String nomineeId = checkingClientHasNominee(loanId);
...
/*        if(CommonMethodsUtil.isNotBlank(loan.client().mobileNo())&&CommonMethodsUtil.isNotBlank(loan.client().getDisplayName())&&CommonMethodsUtil.isNotBlank(amountDisbursed)) {
            ...
            String msg = createDisbursalTemplate(loan, repaymentPeriods, installmentAmount);
            	if(msg != null) {
//            	String templateParamString ="templateParameters[A]="+amountDisbursed.toString()+"&&templateParameters[B]="+loan.getAccountNumber();
            	...
            	SMSDataVIVO smsdata = new SMSDataVIVO("disbursement", loan.client().mobileNo(), msg);
            	this.smsProcessingService.sendSMSinTamil(smsdata);
            	}
            }
        }*/
    ...
    //Habile changes for checking wether the client has nominee details before disbursing loan 
    private String checkingClientHasNominee(final Long loanId)
    {
    	boolean result = false;
    	
        final String sql = "select nd.id from `nominee detail` nd join m_client c on c.id = nd.client_id join m_loan l on l.client_id = c.id where l.id = " + loanId + " limit 1 ";

        final SqlRowSet rs = this.jdbcTemplate.queryForRowSet(sql);

        String containsnominee = null;
        
        if (rs.next())
        {
        	//nomineeId = rs.getString("nd.id"); 	
        	containsnominee = "Client has nominee details";
        } 
        else
        {
        	throw new GeneralPlatformDomainRuleException("Nominee detail is missing. Please add the nominee details.",
                    "Loan cannot be disbursed without the nominee details.");
        }

        return containsnominee;
    }
    
    
/*    private String createDisbursalTemplate(final Loan loan, List<LoanSchedulePeriodData> repaymentPeriods, BigDecimal installmentAmount)
    {	
    	String Message = null;
    	
    	BigDecimal loanAmount = loan.getApprovedPrincipal();
    	BigDecimal processingFee = ((LoanSchedulePeriodData) repaymentPeriods.get(0)).feeChargesPaid();
    	BigDecimal disbursedAmount = loanAmount.subtract(processingFee);
     	BigDecimal rateOfInterest = loan.getLoanProductRelatedDetail().getAnnualNominalInterestRate();
    	Integer noOfRepayments = loan.getTermFrequency();
    	
        String sql = "select CONCAT(`Bank Name` ,\",\", `Account Number`) from `bank detail` where client_id = ?";

        String sample = (String) jdbcTemplate.queryForObject(
                sql, new Object[] { loan.getClientId() }, String.class);
        
        String[] sampleList = sample.split(",");
 
    	String bank = sampleList[0];
    	String accountNo = sampleList[1].trim();
    	
    	
    	String s = "*";
    	int n = accountNo.length();
    	String code = IntStream.range(0, n).mapToObj(i -> s).collect(Collectors.joining(""));
    	String substr = accountNo.substring(accountNo.length() - 4);
    	String Ecrypt = code+substr;
    	
    	if(bank != null || Ecrypt != null)
    	{
    	Message = "தங்களது கடன் விண்ணப்பத்தை ஏற்று, தாங்கள் கோரி உள்ள கடன் தொகையான ரூபாய் " + loanAmount.toBigInteger()
    	+" அனுமதிக்கப்பட்டுஉள்ளது. இதில் காப்பீடு மற்றும் பரிசீலனை தொகையாக ரூபாய் " + processingFee.toBigInteger()
    	+" பிடித்தம் போக, ரூபாய் " + disbursedAmount.toBigInteger()
    	+" தங்களது " + bank
    	+" வங்கி சேமிப்பு கணக்கு எண் " + Ecrypt
    	+" இற்கு இரண்டு நாட்களில் அனுப்பப்படும். இதற்கான வட்டி ஆண்டிற்கு " + rateOfInterest.toBigInteger()
    	+" சதவிகிதம். இந்த கடன் தொகையை மாதம் ரூபாய் " + installmentAmount.toBigInteger()
    	+" விகிதம் " + noOfRepayments.toString()
    	+" மாதங்களில் திருப்பி செலுத்தவேண்டும். - விவர்தனா மைக்ரோபைனான்ஸ் லிமிடெட். ";
    	}
     	LoanProduct loanprod = loan.getLoanProduct();
    	 Collection<LoanCharge> loanch = loan.getLoanCharges();
    	 List<LoanDisbursementDetails> dis = loan.getDisbursementDetails();
    	 List<DisbursementData> dis2 = loan.getDisbursmentData();
     	String sample = loan.getLoanProductRelatedDetail().getNominalInterestRatePerPeriod().toPlainString();

        return Message;
    }*/
    
    
    private String createRepaymentTemplate(Loan loan, BigDecimal transactionAmount, long loanID, Integer Installment_number, BigDecimal installmentAmount, int type)
    {	
    	String Message = null;
    	
        String sql = "select max(lrs.installment) from m_loan_repayment_schedule lrs where lrs.loan_id = ? and lrs.obligations_met_on_date is not null";
        Integer installment = this.jdbcTemplate.queryForObject(sql, new Object[] { loanID }, Integer.class);
        Integer noOfRepayments = loan.getTermFrequency();
        if(installment == null) {
        	installment = 1;
        }
        ...
        int remaining = noOfRepayments - Installment_number;

        if(type == 1)
        {
        
    	Message = "தங்களது " + Installment_number
    			+ "ம் தவணை தொகை ரூபாய் " + transactionAmount.toBigInteger() 
    			+ " கடன் நிலுவையில் வரவு வைக்கப்பட்டது. தற்பொழுது தங்களது கடன் கணக்கில் இன்னும் " + remaining
    			+ " மாத தவனைகள் பாக்கி உள்ளன. - விவர்தனா மைக்ரோபைனான்ஸ் லிமிடெட். ";
        }
        else if(type == 2) {
        	BigDecimal remaining_amt = installmentAmount.subtract(transactionAmount);
        	Message = "தங்களது  " + Installment_number
        			+ " தவணை தொகை ரூபாய்  " + installmentAmount.toBigInteger()
        			+ " செலுத்த வேண்டும் அதில் இன்று ரூபாய்  " + transactionAmount.toBigInteger() 
        			+ " மட்டுமே கடன் நிலுவையில் வரவு வைக்கப்பட்டது. பாக்கி ரூபாய்  " + remaining_amt.toBigInteger()
        			+ " இன்னும் நிலுவையில் உள்ளது.  இது தவிர தங்களது கடன் கணக்கில் இன்னும்  " + remaining 
        			+ " மாத தவணைகள் பாக்கி உள்ளன. - விவர்தனா மைக்ரோபைனான்ஸ் லிமிடெட். ";

        }
        else if(type == 3) {
        	BigDecimal excess_amt = transactionAmount.subtract(installmentAmount);
        	Message = "தங்களது " + Installment_number
        			+ " ம் தவணை தொகை ரூபாய்  " + installmentAmount.toBigInteger() 
        			+ " செலுத்த வேண்டும். அதில் இன்று ரூபாய் " + excess_amt.toBigInteger()
        			+ " கூடுதலாகச் செலுத்தியுள்ளீர்கள்.  இது தங்கள் கணக்கில் வரவு வைத்து நேர்செய்யப்படும். இது தவிர தங்களது கடன் கணக்கில் இன்னும் "
        			+ remaining + " மாத தவணைகள் பாக்கி உள்ளன. - விவர்தனா மைக்ரோபைனான்ஸ் லிமிடெட். ";	
        }
        
        return Message;
    }
    
    private String createRepaymentTemplate2(Loan loan, BigDecimal transactionAmount, long loanID)
    {	
    	String Message = null;
        ...
        int remaining = noOfRepayments - installment;
        
    	Message = "தங்களது " + installment
    			+ "ம் தவணை தொகை ரூபாய் " + transactionAmount.toBigInteger() 
    			+ " கடன் நிலுவையில் வரவு வைக்கப்பட்டது. தற்பொழுது தங்களது கடன் கணக்கில் இன்னும் " + remaining
    			+ " மாத தவனைகள் பாக்கி உள்ளன. - விவர்தனா மைக்ரோபைனான்ஸ் லிமிடெட். ";
...
//        new edit
   	 LoanScheduleData repaymentPrevSchedule = null;
     LoanAccountData loanPrevBasicDetails = this.loanReadPlatformService.retrieveOne(loanId);
        repaymentPrevSchedule = this.loanReadPlatformService.retrieveRepaymentSchedule(loanId, loanPrevBasicDetails.repaymentScheduleRelatedData(),
         		this.loanReadPlatformService.retrieveLoanDisbursementDetails(loanId), loanPrevBasicDetails.isInterestRecalculationEnabled(), loanPrevBasicDetails.getTotalPaidFeeCharges());	
         List<LoanSchedulePeriodData> repaymentOriginalPeriods = new ArrayList<LoanSchedulePeriodData> (repaymentPrevSchedule.getPeriods()) ;
         BigDecimal installmentAmount =  ((LoanSchedulePeriodData) repaymentOriginalPeriods.get(1)).getTotalDueForPeriod();
      ...
      //Code to send SMS Starts
         ...
         if(CommonMethodsUtil.isNotBlank(loan.client().mobileNo())&&CommonMethodsUtil.isNotBlank(loan.client().getDisplayName())&&CommonMethodsUtil.isNotBlank(transactionAmount)) {
...
//        	 String templateParamString = "templateParameters[A]="+loan.getAccountNumber()+"&templateParameters[B]="+transactionAmount.toString();
             ...
             repaymentSchedule = this.loanReadPlatformService.retrieveRepaymentSchedule(loanId, loanBasicDetails.repaymentScheduleRelatedData(),
             		this.loanReadPlatformService.retrieveLoanDisbursementDetails(loanId), loanBasicDetails.isInterestRecalculationEnabled(), loanBasicDetails.getTotalPaidFeeCharges());	
             List<LoanSchedulePeriodData> repaymentPeriods = new ArrayList<LoanSchedulePeriodData> (repaymentSchedule.getPeriods()) ;
             Integer Installment_number = null;
             int type = 1;

             if(CommonMethodsUtil.isNotBlank(repaymentPeriods)&& repaymentPeriods.size()>1) {
            	 LocalDate prevDate;
            	 
            	 for (final LoanSchedulePeriodData singleRepaymentPeriod : repaymentPeriods) {
            	 if(singleRepaymentPeriod.getComplete() != null) {	
            		 if(!singleRepaymentPeriod.getComplete()) {
            			 BigDecimal outStanding = singleRepaymentPeriod.getTotalDueForPeriod().subtract(singleRepaymentPeriod.principalPaid().add(singleRepaymentPeriod.interestPaid().add(singleRepaymentPeriod.feeChargesPaid().add(singleRepaymentPeriod.penaltyChargesPaid()))));
            		 if(outStanding.signum() == 1 && !outStanding.equals(singleRepaymentPeriod.getTotalDueForPeriod())) {
            			if(Installment_number == null ) {
            				Installment_number = singleRepaymentPeriod.periodNumber();
            				type = 2;
            			}
            		 ...
            		 {
            			 prevDate = singleRepaymentPeriod.getObligationsMetOnDate();
            			 if(transactionDate.equals(prevDate)) {
            				 Installment_number = singleRepaymentPeriod.periodNumber();
            				 }
            			 installmentAmount =  ((LoanSchedulePeriodData) repaymentOriginalPeriods.get(singleRepaymentPeriod.periodNumber())).getTotalDueForPeriod();
        				 BigDecimal exceed = installmentAmount.subtract(singleRepaymentPeriod.principalPaid().add(singleRepaymentPeriod.interestPaid().add(singleRepaymentPeriod.feeChargesPaid().add(singleRepaymentPeriod.penaltyChargesPaid()))));
        				 if(exceed.signum() == -1){
        					 Installment_number = singleRepaymentPeriod.periodNumber();
        					 type = 3;
        				 }
             ...
             installmentAmount =  ((LoanSchedulePeriodData) repaymentOriginalPeriods.get(Installment_number)).getTotalDueForPeriod();
        	 String msg = createRepaymentTemplate(loan, transactionAmount, loanId , Installment_number, installmentAmount, type);
        	 ...
        	 SMSDataVIVO smsdata = new SMSDataVIVO("repayment", loan.client().mobileNo(), msg);
        	 this.smsProcessingService.sendSMSinTamil(smsdata);
         	}
         	...
         	//Code to send SMS Starts
         	if(CommonMethodsUtil.isNotBlank(loan.client().mobileNo())&&CommonMethodsUtil.isNotBlank(loan.client().getDisplayName())&&CommonMethodsUtil.isNotBlank(loan.getClosedOnDate())&&CommonMethodsUtil.isNotNull(loan.getClosedOnDate())) {
...
//              new edit
                ...
                Long loanId = singleLoanRepaymentCommand.getLoanId();
//                final DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
                Date date = singleLoanRepaymentCommand.getTransactionDate().toDate();
                java.sql.Date sDate = new java.sql.Date(date.getTime());
//                String transactionDate = df.format(date.toDate());
                MapSqlParameterSource paramSource = new MapSqlParameterSource();
                paramSource.addValue("loanId", loanId);
                paramSource.addValue("date", sDate);
                SqlRowSet rs = jdbcTemplate.queryForRowSet("select getAllPrincipleDueOnDate(:loanId,:date) + getAllInterestDueOnDate(:loanId,:date) + getAllFeesDueOnDate(:loanId,:date) + getAllPenaltyDueOnDate(:loanId,:date) as totaldue;", paramSource);
               
                BigDecimal transactionAmt = singleLoanRepaymentCommand.getTransactionAmount();
                BigDecimal installmentAmount = null;
                if (rs.next()) {
                	installmentAmount = rs.getBigDecimal("totaldue") ;
                }
                
                int type = 1;
                
                BigDecimal teller = installmentAmount.subtract(transactionAmt);
                if(teller.signum() == 1) {
                	type = 2;
                }
                else if(teller.signum() == -1) {
                	type = 3;
                }
                else {
                	type = 1;
                ...
                if(CommonMethodsUtil.isNotBlank(loan.client().mobileNo())&&CommonMethodsUtil.isNotBlank(loan.client().getDisplayName())&&CommonMethodsUtil.isNotBlank(singleLoanRepaymentCommand.getTransactionAmount())) {
               	 // String templateParamString = "templateParameters[A]="+loan.client().getDisplayName()+"&templateParameters[B]="+loan.getAccountNumber()+"&templateParameters[C]="+loan.getCurrencyCode()+"&templateParameters[D]="+transactionAmount.toString()+"&templateParameters[E]="+transactionDate.toString("dd/MM/yyyy")+"&templateParameters[F]="+loan.getSummary().getTotalOutstanding().toString()+"&templateParameters[G]="+loan.getCurrencyCode();
//               	 String templateParamString = "templateParameters[A]="+loan.getAccountNumber()+"&templateParameters[B]="+transactionAmount.toString();
                    ...
                    List<LoanSchedulePeriodData> repaymentPeriods = new ArrayList<LoanSchedulePeriodData> (repaymentSchedule.getPeriods()) ;
                    Integer Installment_number = null;
                    BigDecimal installmentAmount2 = installmentAmount;
                    ...
                    if(CommonMethodsUtil.isNotBlank(repaymentPeriods)&& repaymentPeriods.size()>1) {
                   	 ...
                   	 if(singleRepaymentPeriod.getComplete() != null) {	
                   		 if(!singleRepaymentPeriod.getComplete()) {
                   			 BigDecimal outStanding = installmentAmount.subtract(singleRepaymentPeriod.principalPaid().add(singleRepaymentPeriod.interestPaid().add(singleRepaymentPeriod.feeChargesPaid().add(singleRepaymentPeriod.penaltyChargesPaid()))));
                   		 if(!isAdvance && outStanding.signum() == 1 && !outStanding.equals(singleRepaymentPeriod.getTotalDueForPeriod())) {
                   			if(Installment_number == null ) {
                   				Installment_number = singleRepaymentPeriod.periodNumber();
                   			}
                   		 ...
                   		 else
                   		 {
                   			 ...
                   			 if(singleLoanRepaymentCommand.getTransactionDate().equals(prevDate)) {
                   				 Installment_number = singleRepaymentPeriod.periodNumber();
                   				 installmentAmount2 = exceed;
                   				 isAdvance = true;
                   			 }
                   			prevDate = singleRepaymentPeriod.getObligationsMetOnDate();
                   			
               				 exceed = installmentAmount2.subtract(singleRepaymentPeriod.principalPaid().add(singleRepaymentPeriod.interestPaid().add(singleRepaymentPeriod.feeChargesPaid().add(singleRepaymentPeriod.penaltyChargesPaid()))));
               				 if(exceed.signum() == -1 || isAdvance){
               					 Installment_number = singleRepaymentPeriod.periodNumber();
               				 }
               				 else {
               					Installment_number = singleRepaymentPeriod.periodNumber();
               	 ...
               	 String msg = createRepaymentTemplate(loan, singleLoanRepaymentCommand.getTransactionAmount(), loan.getId() , Installment_number, installmentAmount, type);
            	 // String templateParamString = "templateParameters[A]="+loan.getAccountNumber()+"&templateParameters[B]="+loan.getCurrencyCode()+"&templateParameters[C]="+transactionAmount.toString()+"&templateParameters[D]="+transactionDate.toString("dd/MM/yyyy");
            	 SMSDataVIVO smsdata = new SMSDataVIVO("repayment", loan.client().mobileNo(), msg);
            	 this.smsProcessingService.sendSMSinTamil(smsdata);
             	}              
    ...
    public CommandProcessingResult forecloseLoan(final Long loanId, final JsonCommand command) {
      ...
      //Code to send SMS Starts
 if(CommonMethodsUtil.isNotBlank(loan.client().mobileNo())&&CommonMethodsUtil.isNotBlank(loan.client().getDisplayName())&&CommonMethodsUtil.isNotBlank(loan.getClosedOnDate())) {
	 	Date todayDate = loan.getClosedOnDate();
	 	SimpleDateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy");
	 	String actualClosedDate = dateFormat.format(todayDate);  
	 	BigDecimal loanamt = loan.getApprovedPrincipal();
