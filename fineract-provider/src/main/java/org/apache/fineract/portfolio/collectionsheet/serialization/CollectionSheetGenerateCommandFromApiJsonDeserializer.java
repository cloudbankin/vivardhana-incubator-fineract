...
public class CollectionSheetGenerateCommandFromApiJsonDeserializer {
	...
	final Set<String> supportedParameters = new HashSet<>(
			Arrays.asList(transactionDateParamName, localeParamName, dateFormatParamName, calendarIdParamName, repaymentDateParamName));
    ...
    public void validateForGenerateCollectionSheet(final String json) {
        ...
        final String repaymentDateStr = this.fromApiJsonHelper.extractStringNamed(repaymentDateParamName, element);
        ...
        {
        	if (!StringUtils.isBlank(repaymentDateStr)) 
        	{
                final LocalDate dueDate = this.fromApiJsonHelper.extractLocalDateNamed(repaymentDateParamName, element);
            }
        }
