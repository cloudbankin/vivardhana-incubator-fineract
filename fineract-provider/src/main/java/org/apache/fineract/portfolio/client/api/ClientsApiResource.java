...
public class ClientsApiResource {
    ...
    private final ToApiJsonSerializer<AdvanceClientData> advancetoApiJsonSerializer;
    ...
    public ClientsApiResource(final PlatformSecurityContext context, final ClientReadPlatformService readPlatformService,
            final ToApiJsonSerializer<ClientData> toApiJsonSerializer,final ToApiJsonSerializer<AdvanceClientData> advancetoApiJsonSerializer,
        ...
        this.advancetoApiJsonSerializer = advancetoApiJsonSerializer;
    }
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String advanceRetrieveAll(@Context final UriInfo uriInfo, @QueryParam("sqlSearch") final String sqlSearch,
    		@QueryParam("officeId") final Long officeId,@QueryParam("fund") final Long fund,@QueryParam("loanPurpose") final Long loanPurpose,
    		@QueryParam("loanOfficer") final Long loanOfficer,@QueryParam("loanProduct") final Long loanProduct,@QueryParam("displayName") final String displayName,
    		@QueryParam("mobileNo") final String mobileNo,@QueryParam("status") final Long status,
    		@QueryParam("offset") final Integer offset, @QueryParam("limit") final Integer limit) {
    	
    	return this.advancesearch(uriInfo, sqlSearch, officeId, fund,
    			loanPurpose, loanOfficer, loanProduct, displayName, mobileNo, status,offset,limit);
    	
    }
    
    //Habile changes Advance Search
    
    public String advancesearch(final UriInfo uriInfo,final String sqlSearch,
    	...
    	final AdvanceSearchParameters advanceSearchParameters = AdvanceSearchParameters.forClientsAdvanceSearch(sqlSearch, officeId, fund,
    			loanPurpose, loanOfficer, loanProduct, displayName, mobileNo, status, offset, limit);
    	
    	final Page<ClientData> clientData = this.clientReadPlatformService.advanceRetrieveAll(advanceSearchParameters);
    ...
    @GET
    @Path("advanceTemplate")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String retrieveAdvanceTemplate(@Context final UriInfo uriInfo, @QueryParam("officeId") final Long officeId,
            @QueryParam("commandParam") final String commandParam,
            @DefaultValue("false") @QueryParam("staffInSelectedOfficeOnly") final boolean staffInSelectedOfficeOnly) {
    	//
    	AdvanceClientData advanceClientData = null;

    	advanceClientData = this.clientReadPlatformService.advanceRetrieveTemplate(officeId, staffInSelectedOfficeOnly);
    	
    	final ApiRequestJsonSerializationSettings settings = this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
        return this.advancetoApiJsonSerializer.serialize(settings, advanceClientData, ClientApiConstants.ADVANCE_CLIENT_RESPONSE_DATA_PARAMETERS);
    ...
    //High Mark Newly added api call 
    @GET
    @Path("/highMark")
    @Consumes({ MediaType.APPLICATION_JSON })
		...
		try 
		{
			...
			SimpleDateFormat sdf3 = new SimpleDateFormat("EEE MMM dd yyyy HH:mm:ss zzz ", Locale.ENGLISH);
		   	  ...
		   	  try 
		   	  {
					 ...
					 excelDate = format1.format(tempDate);
			  }
            ...
            File newFile = new File(excelDate+"_VIVARDHANA_MICRO_FINANCE_MONTHLY_DATA.xls");
            result.write(new FileOutputStream(newFile));
			final ResponseBuilder response = Response.ok(new FileInputStream(newFile));
	        response.header("Content-Disposition", "attachment; filename=\"" + excelDate+"_VIVARDHANA_MICRO_FINANCE_MONTHLY_DATA.xls"+ "\"");
    ...
    // High mark weekly submission report
    @GET
    @Path("/weeklyHighMark")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_OCTET_STREAM })
    public Response retrieveWeeklyLoanSchedule(@QueryParam("asOnDate") final String asOnDate, @QueryParam("closedFrom") final String closedFrom, @Context final UriInfo uriInfo) {

        this.context.authenticatedUser().validateHasReadPermission(ClientApiConstants.CLIENT_RESOURCE_NAME);
        Workbook result =this.clientReadPlatformService.retrieveWeeklyHighMarkData(asOnDate, closedFrom);
		...
		try 
		{
			...
			File newFile = new File(excelDate+"_VIVARDHANA_MICRO_FINANCE_WEEKLY_DATA.xls");
            result.write(new FileOutputStream(newFile));
			final ResponseBuilder response = Response.ok(new FileInputStream(newFile));
	        response.header("Content-Disposition", "attachment; filename=\"" + excelDate+"_VIVARDHANA_MICRO_FINANCE_WEEKLY_DATA.xls"+ "\"");
	        response.header("Content-Type","application/vnd.ms-excel");
	        return response.build();
  ...
  //Equifax Newly added api call 
    @GET
    @Path("/equifax")
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_OCTET_STREAM })
    public Response retrieveEquifaxLoanSchedule(@QueryParam("asOnDate") final String asOnDate, @QueryParam("closedFrom") final String closedFrom, @Context final UriInfo uriInfo) {

        this.context.authenticatedUser().validateHasReadPermission(ClientApiConstants.CLIENT_RESOURCE_NAME);
        Workbook result =this.clientReadPlatformService.retrieveEquifaxData(asOnDate, closedFrom);
        
		try 
		{		  
            File newFile = new File("Equifax Data Submission.xls");
            result.write(new FileOutputStream(newFile));
			final ResponseBuilder response = Response.ok(new FileInputStream(newFile));
	        response.header("Content-Disposition", "attachment; filename=\""+"Equifax Data Submission.xls"+ "\"");
	        response.header("Content-Type","application/vnd.ms-excel");
	        return response.build();
