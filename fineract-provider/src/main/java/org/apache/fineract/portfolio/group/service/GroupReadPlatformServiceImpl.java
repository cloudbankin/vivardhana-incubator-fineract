...
public class GroupReadPlatformServiceImpl implements GroupReadPlatformService {
    ...
    @Override
	public Page<GroupGeneralData> advanceRetrievePagedAll(AdvanceSearchParameters advanceSearchParameters,final PaginationParameters parameters) {
         ...
         List<Object> paramList = new ArrayList<>(
                 Arrays.asList(hierarchySearchString));
         final String extraCriteria = getGroupExtraCriteriaforAdvanceSearch(this.allGroupTypesDataMapper.schema(), paramList, advanceSearchParameters);
         this.columnValidator.validateSqlInjection(sqlBuilder.toString(), extraCriteria);
         if (StringUtils.isNotBlank(extraCriteria)) {
             sqlBuilder.append(" and (").append(extraCriteria).append(")");
         }
    ...
    private String getGroupExtraCriteriaforAdvanceSearch(String schemaSql, List<Object> paramList, final AdvanceSearchParameters advanceSearchParameters) {

        StringBuffer extraCriteria = new StringBuffer(200);
        extraCriteria.append(" and g.level_Id = ").append(GroupTypes.GROUP.getId());
        String sqlSearch = advanceSearchParameters.getSqlSearch();
        ...
        final String externalId = advanceSearchParameters.getGroupExternalId();
        if (externalId != null) {
        	//paramList.add(ApiParameterHelper.sqlEncodeString(externalId));
