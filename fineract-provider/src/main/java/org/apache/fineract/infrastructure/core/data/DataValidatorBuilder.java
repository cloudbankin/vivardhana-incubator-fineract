...
public class DataValidatorBuilder {
    ...
    public DataValidatorBuilder notBlankForPostalCode() {
        if (this.value == null && this.ignoreNullValue) { return this; }

        if (this.value == null || StringUtils.isBlank(this.value.toString())) {
            String realParameterName = this.parameter;
            
            final StringBuilder validationErrorCode = new StringBuilder("Postal Code should be given. Please add Postal Code.");
    ...
    //Habile changes for validating the mobile number 
    public DataValidatorBuilder isNotValidMobileNo(String mobileNo)
    {
    	if (this.value == null && this.ignoreNullValue) { return this; }
    	
    	if (this.value != null && !mobileNo.matches("^[0-9]{10}$")) {
            final StringBuilder validationErrorCode = new StringBuilder("The ").append(this.resource).append(" ")
                    .append(this.parameter).append(" must contain 10 digits");
            final StringBuilder defaultEnglishMessage = new StringBuilder("The parameter ").append(this.parameter)
                    .append(" exceeds max length of ");
            final ApiParameterError error = ApiParameterError.parameterError(validationErrorCode.toString(),
                    defaultEnglishMessage.toString(), this.parameter, mobileNo, this.value.toString());
            this.dataValidationErrors.add(error);
        }
        return this;
    }

    //Habile changes for validating the external id 
    /*public DataValidatorBuilder validateAadharNumber(String aadharNumber)
    {
    	if (this.value == null && this.ignoreNullValue) { return this; }
    ...
    public DataValidatorBuilder matchesRegularExpressionForPostalCode(final String expression) {
        if (this.value == null && this.ignoreNullValue) { return this; }

        if (this.value != null && !this.value.toString().matches(expression)) {
            final StringBuilder validationErrorCode = new StringBuilder("Invalid pincode. Please enter a valid pincode");
            final StringBuilder defaultEnglishMessage = new StringBuilder("The parameter ").append(this.parameter)
                    .append(" must match the provided regular expression [ ").append(expression).append(" ] ").append(".");
