...
import org.apache.fineract.portfolio.client.data.VerhoeffAlgorithm;
import org.apache.fineract.portfolio.client.exception.ClientAgeInvalidException;
...
public class ReadWriteNonCoreDataServiceImpl implements ReadWriteNonCoreDataService {
    ...
    private CommandProcessingResult updateDatatableEntry(final String dataTableName, final Long appTableId, final Long datatableId,
        ...
        final Map<String, Object> changes = getAffectedAndChangedColumns(grs, dataParams, pkName,dataTableName,appTableId);
    ...
    private String getAddSql(final List<ResultsetColumnHeaderData> columnHeaders, final String datatable, final String fkName,
        ...
        final Map<String, String> affectedColumns = getAffectedColumns(columnHeaders, queryParams, fkName, datatable, appTableId);
    ...
    private Map<String, Object> getAffectedAndChangedColumns(final GenericResultsetData grs, final Map<String, String> queryParams,
            final String fkName,final String dataTableName, final Long appTableId) {

        final Map<String, String> affectedColumns = getAffectedColumns(grs.getColumnHeaders(), queryParams, fkName,dataTableName,appTableId);
    ...
    public Map<String, String> getAffectedColumns(final List<ResultsetColumnHeaderData> columnHeaders,
            ...
            final String datatable,final Long appTableId) {
                ...
                for (final ResultsetColumnHeaderData columnHeader : columnHeaders) {
                    if (notFound) {
                        ...
                        if (queryParamColumnUnderscored.equalsIgnoreCase(columnHeaderUnderscored)) {
                            ...
                            if(queryParamColumnUnderscored.equalsIgnoreCase("IFSC_Code"))
                            {
                            	pValue = validateIFSCcode(columnHeader, pValue, dateFormat, clientApplicationLocale);
                            }
                            if(datatable.equalsIgnoreCase("Nominee detail") &&queryParamColumnUnderscored.equalsIgnoreCase("Aadhaar_number"))
                            {
                            	pValue = checkWhetherClientandNomineeAadhaarNoIsNotSame(columnHeader, pValue, dateFormat, clientApplicationLocale, appTableId);
                            }
    ...
    private String validateColumn(final ResultsetColumnHeaderData columnHeader, final String pValue, final String dateFormat,
        ...
        //Habile changes for making the date of birth mandatory 
        if(StringUtils.isEmpty(paramValue) && columnHeader.getColumnName().equals("Date of birth"))
        {
        ...
        if (StringUtils.isNotEmpty(paramValue)) {
            ...
            if (columnHeader.isDateDisplayType()) {
                ...
                if(columnHeader.getColumnName().equalsIgnoreCase("Date of birth"))
                {
                	
                    Date date1=null;
                		try {
                		date1=new SimpleDateFormat("yyyy-MM-dd").parse(paramValue); 
                		}catch(Exception e)
                		{
                    	...
                    	long diffinms = currentdate.getTime() - date1.getTime();
                    	
                    	long diffDays = diffinms / (24 * 60 * 60 * 1000);
                    	
                    	double diffyears = diffDays/365.25;
                    	
                    	if(diffyears <=18 || diffyears >= 58)
                    	{
                    		final ApiParameterError error = ApiParameterError.parameterError(
                                    "The Nominee's Age is Invalid. Age must be between 18 to 58",
                                    //"The Nominee's Age is Invalid. Age calculated using Date Of Birth must be between 18 to 58",
                                    "The column `" + columnHeader.getColumnName() + "` has  invalid date for Nominee's Age condition",
                ...
                if(columnHeader.getColumnName().equalsIgnoreCase("Mobile Number"))
                {
                	if(!paramValue.matches("^[0-9]{10}$"))
                	{
                		final ApiParameterError error = ApiParameterError.parameterError(
                                "The mobile number must contain 10 digits",
                                "The column `" + columnHeader.getColumnName() + "` exceeds its defined max-length ",
                                columnHeader.getColumnName(), paramValue);
                        final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
                        dataValidationErrors.add(error);
    ...
    //Habile changes for validating the IFSC CODE 
    private String validateIFSCcode(final ResultsetColumnHeaderData columnHeader, final String pValue, final String dateFormat,
            final Locale clientApplicationLocale) {
        ...
        if (StringUtils.isNotEmpty(paramValue)) 
        {
	    	if (columnHeader.isString()) 
	    	{
	            ...
	            /*//Old Regex is [A-Z|a-z]{4}[0][\\d]{6}$
	            else if(!paramValue.matches("^[A-Za-z]{4}[0][a-zA-Z0-9]{6}$"))
	            {
	            	final ApiParameterError error = ApiParameterError.parameterError(
	                        "The first 4 characters must be alphabets, Fifth character must be  0 and the length of the IFSC code must be 11",
	                        "The first 4 characters must be alphabets, Fifth character must be  0 and the length of the IFSC code must be 11",
	                        columnHeader.getColumnName(), paramValue);
	                final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
	                dataValidationErrors.add(error);
	            ...
	            else if(!queryForCheckingIfscCode(paramValue))
	            {
	            	final ApiParameterError error = ApiParameterError.parameterError(
	                        "Invalid IFSC code",
	                        "The column `" + columnHeader.getColumnName() + "` is not a valid ifsc code ",
	                        columnHeader.getColumnName(), paramValue);
	                final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
	                dataValidationErrors.add(error);
	                throw new PlatformApiDataValidationException("validation.msg.validation.errors.exist", "Validation errors exist.",
    ...
    //Habile changes for validating the IFSC CODE 
    private boolean queryForCheckingIfscCode(final String paramValue)
    {
    	boolean result = false;
    	
        final String sql = "select IFSC FROM ifsc_codes where IFSC = '" + paramValue + "'";
    ...
    //Habile changes for validating the nominee aadhaar no
    private String checkWhetherClientandNomineeAadhaarNoIsNotSame(final ResultsetColumnHeaderData columnHeader, final String pValue, final String dateFormat,
            final Locale clientApplicationLocale,final Long appTableId) {
        ...
        if (StringUtils.isNotEmpty(paramValue)) 
        {
	    	if (columnHeader.isString()) 
	    	{
	            ...
	            if (!paramValue.matches("^[0-9]{12}$")) 
	            {
	                final ApiParameterError error = ApiParameterError.parameterError(
	                        "Aadhaar number must contain 12 digits",
	                        "Aadhaar number`" + columnHeader.getColumnName() + "` must contain 12 digits ",
	                        columnHeader.getColumnName(), paramValue);
	                final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
	                dataValidationErrors.add(error);
	                throw new PlatformApiDataValidationException("validation.msg.validation.errors.exist", "Validation errors exist.",
	            ...
	            if(!queryForCheckingAadhaarNo(appTableId, paramValue))
	            {
	            	final ApiParameterError error = ApiParameterError.parameterError(
	                        "Client and nominee aadhaar number must not be same",
	                        "Client and nominee`" + columnHeader.getColumnName() + "` aadhaar number must not be same ",
	                        columnHeader.getColumnName(), paramValue);
	                final List<ApiParameterError> dataValidationErrors = new ArrayList<>();
	                dataValidationErrors.add(error);
	                throw new PlatformApiDataValidationException("validation.msg.validation.errors.exist", "Validation errors exist.",
	            ...
	            boolean result = VerhoeffAlgorithm.validateVerhoeff(paramValue);
	            if(!result)
	            {
	            	final ApiParameterError error = ApiParameterError.parameterError(
    ...
    //Habile changes for validating the nominee aadhaar no 
    private boolean queryForCheckingAadhaarNo(final Long appTableId,final String nomineeAadhaarNo)
    {
    	boolean result = false;
    	
        final String sql = "select external_id from m_client c where c.id = '" + appTableId + "'";

        final SqlRowSet rs = this.jdbcTemplate.queryForRowSet(sql);

        String clientAadhaarNo = null;
        
        if (rs.next())
        {
        	clientAadhaarNo = rs.getString("external_id");
        	
        	if(!clientAadhaarNo.equals(nomineeAadhaarNo))
        	{
        		result = true;
        	}
        	else
